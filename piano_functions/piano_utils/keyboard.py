...
def parser():
    ...
    parser.add_argument("--gamma", type=int, default=0.5,
                            help="光照补偿系数,越小就越亮")
    parser.add_argument("--video_path", type=str, default="/home/data/cy/projects/piano/KJnotes/2音协二级 二月里来.mp4",
                            help="视频存储路径")                                               
    args = parser.parse_args()
    return args

# test_list = [os.path.join(args.save_path, x) for x in file_list if x.startswith("0003")] #--测试图片list
...
#----得到包含钢琴的矩形框------
def find_paino(img,img_path, crop_path,thres,binary_thres):
...
#----得到包含钢琴的矩形框------
def find_paino1(img,img_path, crop_path,thres,binary_thres):
...
#----用以别的函数进行调用---
def find_Rect(img_list,crop_path,w_thres,binary_thres,adjustNum,adj_light):
    Rectlist = []
    frames = 0
    Tostop = 1    #------在测试的时候为了加快速度,因此就直接break
    begin_nums = []   #----如果这种方法得到第一次检测钢琴图片不准,可以加个函数,重新遍历,取第一次得到的rect等于最终rect的那个下标作为背景index
    ...
    # for img_path in test_list:
        ...
        img1 = img.copy()
        Rect, Rect1 = find_paino(img, img_path, crop_path,
                                 w_thres,binary_thres)
        ...
        if Rect is not None:
            ...
            flag = adjust_thres(Rect, Rect1,adjustNum)
            #----判断是否调整每行的积累的阈值
            new_thres = 0.35
            if flag:
                print("adjust")
                Rect, Rect1 = find_paino(img, img_path, crop_path,
                                         new_thres,binary_thres)  
            
            light = adjust_light(Rect, Rect1, adj_light)
            #----判断是否调整光照强度   #-----当heigth比较小的时候也可以提升一下光照看一下
            if light:
                print("light")
                ...
                new_light = 130
                Rect, Rect1 = find_paino1(img, img_path, crop_path,
                                         new_thres,new_light)
            #----画图观察
            ...
            cv2.imwrite('/home/cy/projects/github/project_piano/sfd_hand/piano_functions/imgs/final/{}.jpg'.format(os.path.basename(img_path)), crop_piano)
        if frames > Tostop:
            break
        end = time.time()
        print("矩形框为{}".format(Rect))
        print("当前帧 {} cost {:.2f} S ".format(img_path,end-begin))
    ...
    if (len(x) > 0):   
        ...
        Rect = x[0][0]
    ori_Rect = (Rect[0] + Rect1[0], Rect[1] + Rect1[1], Rect[2] + Rect1[0], Rect[3] + Rect1[1])
    #--(x,y,x+w,x+h)
    # print("最终的Rect为{}".format(ori_Rect))
    
    return ori_Rect, begin_num
...
#-----每个视频中都挑选一张图像出来测试得到钢琴box的效果怎么样-----
    videos_path = '/home/data/cy/projects/piano/data/imgs/'
    videos_list=[os.path.join(videos_path,x) for x in os.listdir(videos_path)]
    test_img = []
    for path1 in videos_list:
        img_list = [os.path.join(path1, x) for x in os.listdir(path1)]
        img_list.sort(key=lambda x: str(x))
        test_img.append(img_list[150])
    test_img.sort(key=lambda x: str(x))
    error_img = []  #----记录检测不到钢琴box的
    

    file_list = [x for x in os.listdir(args.save_path) if x.endswith((".jpg", ".png"))]
    file_list.sort(key=lambda x: int(x[:-4]))
    img_list = [os.path.join(args.save_path, x) for x in file_list]
    ...
    # for img_path in test_list:
        ...
        img1 = np.zeros(img.shape, np.uint8)
        img1 = img.copy()
        Rect, Rect1 = find_paino(img, img_path, args.crop_path,
        ...
        if Rect is not None:
            ...
            if light:
                ...
                #-----先进行霍夫变化在调整光照(只对二值化有作用)
                new_light = 130
                Rect, Rect1 = find_paino1(img, img_path,  args.crop_path,
                                         new_thres,new_light)
            #----画图观察
            crop_piano = img1[Rect1[1]:Rect1[3], Rect1[0]:Rect1[2]]  #----霍夫变化得到的裁剪图像
            cv2.rectangle(crop_piano, (Rect[0], Rect[1]), (Rect[2], Rect[3]), (0, 255, 0), 2)
            # cv2.imwrite(os.path.join(args.crop_path,os.path.basename(img_path)), crop_piano)
            #---对于字符串的分隔,一般首先都会用strip()函数将开头结尾的空格\n去掉,这样分隔后的list中没有空
            cv2.imwrite('./imgs/final/{}.jpg'.format(img_path.strip().split('/')[-2]), crop_piano)
        
        print("矩形框为{}".format(Rect))
        if Rect is None:
            error_img.append(img_path.strip().split('/')[-2])
        frames += 1
        if (frames > Tostop):
            break
        
    print('error video is {}'.format(error_img))
    dicRect = dict(Counter(Rectlist))   #-----统计出现最多次数的矩形框作为最终的Rect
    x = [(key, value) for key, value in dicRect.items() if value > 1]
    if (len(x) > 0):   
        x.sort(key=lambda x: int(x[1]), reverse=True)
