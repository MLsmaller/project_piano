    ...
    def __init__(self, alpha, batch_size, num_classes, num_features):
        ...
        def __graph__():
            ...
            with tf.name_scope('input'):
                # [BATCH_SIZE, NUM_FEATURES] ,shape中的None可以设置为不同的Batch_size
            ...
            first_conv_weight = self.weight_variable([5, 5, 1, 32]) #---1为输入维度,32为输出维度
            first_conv_bias = self.bias_variable([32])

            input_image = tf.reshape(x_input, [-1, 28, 28, 1])  #--后面的是rowsxcolsxchannels
            ...
            with tf.name_scope('softmax'):   #tf.nn.softmax_cross_entropy_with_logits()求交叉熵函数
                loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=output, labels=y_input))
            tf.summary.scalar('loss', loss)   #---画图函数

            optimizer = tf.train.AdamOptimizer(learning_rate=alpha).minimize(loss)

            with tf.name_scope('accuracy'):
                output = tf.identity(tf.nn.softmax(output), name='prediction')  #--tf.identity()返回一个和输入一样的tensor
                correct_prediction = tf.equal(tf.argmax(output, 1), tf.argmax(y_input, 1))  
                #tf.argmax(x,axis)返回tensor在某一维上数据最大值所在的索引值，0按列计算、1按行计算
                #tf.equal(x,y)判断x,y是否相等,相等返回true,否则为false
                with tf.name_scope('accuracy'):
